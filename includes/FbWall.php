...
public $postcount = 10; 
	...
	$loadmore=" and M.msg_id<$lastid ";
	    ...
	    $query = mysql_query("(SELECT M.msg_id, M.uid_fk, M.message, M.created, U.fullname,M.uploads,M.profile_uid FROM messages M, users U  WHERE M.uid_fk=U.uid and M.uid_fk IN ($user_ids) $loadmore) UNION (SELECT M.msg_id, M.uid_fk, M.message, M.created, U.fullname,M.uploads,M.profile_uid FROM messages M, users U  WHERE M.uid_fk=U.uid and M.profile_uid IN ($user_ids) $loadmore) order by msg_id desc limit " .$this->postcount) or die(mysql_error());
	  ...
	  public function Total_Updates($user_ids) 
	{
	    ...
	    $query = mysql_query("(SELECT M.msg_id, M.uid_fk, M.message, M.created, U.fullname,M.uploads,M.profile_uid FROM messages M, users U  WHERE M.uid_fk=U.uid and M.uid_fk IN ($user_ids) $loadmore) UNION (SELECT M.msg_id, M.uid_fk, M.message, M.created, U.fullname,M.uploads,M.profile_uid FROM messages M, users U  WHERE M.uid_fk=U.uid and M.profile_uid IN ($user_ids) $loadmore) order by msg_id desc") or die(mysql_error());
	...
	$query='';
	  ...
	  $query="limit $second_count,2";
	    $query = mysql_query("SELECT C.com_id, C.uid_fk, C.comment, C.created, U.fullname FROM comments C, users U WHERE C.uid_fk=U.uid and C.msg_id_fk='$msg_id' order by C.com_id asc $query") or die(mysql_error());
	   while($row=mysql_fetch_array($query))
	    $data[]=$row;
        if(!empty($data))
		{
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
        ...
        if ($update!=$result['message']) {
		  ...
		  $uploads=implode(',',array_unique($uploads_array));
            $query = mysql_query("INSERT INTO `messages` (message, uid_fk, ip,created,uploads,profile_uid) VALUES (N'$update', '$uid', '$ip','$time','$uploads','$profile_uid')") or die(mysql_error());
            $newquery = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, U.fullname FROM messages M, users U where M.uid_fk=U.uid and M.uid_fk='$uid' order by M.msg_id desc limit 1 ");
	...
	$comment=mysql_real_escape_string($comment);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
		...
		if ($comment!=$result['comment']) {
            ...
            $newquery = mysql_query("SELECT C.com_id, C.uid_fk, C.comment, C.msg_id_fk, C.created, U.fullname FROM comments C, users U where C.uid_fk=U.uid and C.uid_fk='$uid' and C.msg_id_fk='$msg_id' order by C.com_id desc limit 1 ");
            $result = mysql_fetch_array($newquery);
